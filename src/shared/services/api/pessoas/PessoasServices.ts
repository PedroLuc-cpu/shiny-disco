  ...
  try {
    ...
    if (data) {
      return {
        data,
        totalCount: Number(
          headers["x-total-count"] || Environments.LIMITE_LINE,
        ),
      };
    }
...
const getById = async (id: number): Promise<IDetalhePessoa | Error> => {
  try {
    ...
    console.log(error);
    return new Error(
      (error as { message: string }).message || "Erro de listar o registro",
    );
  }
};
...
const create = async (
  ...
  try {
    ...
    console.error(error);
    return new Error(
      (error as { message: string }).message || "Erro de realizar cadastro.",
    );
  }
};
  ...
  try {
    ...
    return new Error(
      (error as { message: string }).message ||
        "Erro de realizar atualização do registro",
    );
  }
};
...
const deleteById = async (id: number): Promise<IDetalhePessoa | Error> => {
  try {
    ...
    return new Error(
      (error as { message: string }).message ||
        "Erro de realizar remoção do registro.",
    );
  }
};
